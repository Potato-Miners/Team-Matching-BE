package com.teammatching.demo.presentation.Team;
...
import com.teammatching.demo.application.Team.TeamService;
...
public class TeamController {
    ...
    @Operation(
            summary = "팀 리스트 카테고리별 간단 조회",
            description = "카테고리별로 간단한 팀 정보를 포함한 리스트를 제공합니다."
    )
    @ResponseStatus(HttpStatus.OK)
    @GetMapping("/category")
    public ResponseResult<Page<TeamDto.SimpleResponse>> getSimpleTeamsByCategory(
        ...
        return ResponseResult.<Page<TeamDto.SimpleResponse>>builder()
                .resultCode(HttpStatus.OK.value())
                .resultMessage(ResponseMessage.SUCCESS_GET_SIMPLE_TEAMS)
                .resultData(teamService.getSimpleTeamsByCategory(Category.from(category), pageable)
                        .map(TeamDto.SimpleResponse::from))
                .build();
    }
